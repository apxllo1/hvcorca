...
function Selection() {
	...
	const playerSelectedName = useAppSelector((state) => state.dashboard.apps.playerSelected);

	const sortedPlayers = useMemo(() => {
		const selected = players.find((p) => p.Name === playerSelectedName);
		const sorted = players
			.filter((p) => p.Name !== playerSelectedName && (p !== Players.LocalPlayer || IS_DEV))
